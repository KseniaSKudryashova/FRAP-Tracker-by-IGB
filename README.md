...
FRAP-Tracker BASIC handles experiments where only the nucleus moves while the bleached foci remains fixed relative to nuclear boundaries (e.g., histones). The program tracks nucleus movement using phase correlation and calculates foci position as a fixed offset from the nucleus.
